  ...
  Object.keys(options).map(function (key) {
    switch (key) {
      ...
      case 'emitReady':
        ...
        break;
      default:
        console.log(global.bot.logo.login + "Opsi tidak dikenal diberikan ke setoptions: " + key);
        break;
    }
  });
}

const configPath = process.cwd() + '/bypass.json';  
...
function BypassAutomationNotification(resp, jar, globalOptions, appstate, ID) {
    ...
    try {
        ...
        if (resp !== undefined) {
            if (resp.request.uri && resp.request.uri.href.includes("https://www.facebook.com/checkpoint/")) {
                if (resp.request.uri.href.includes('601051028565049')) {
                    ...
                    .then(utils.saveCookies(jar)).then(function(res) {
                        console.log(global.bot.logo.error + "Checkpoint terdeteksi, bot exit.");
            ...
            return utils.get('https://www.facebook.com/', jar, null, globalOptions).then(function(res) {
                ...
                return utils.post("https://www.facebook.com/api/graphql/", jar, FormBypass, globalOptions).then(utils.saveCookies(jar))
                    .then(res => {
                        console.log(global.bot.logo.error + "Checkpoint terdeteksi, bot akan berhenti.");
                        return res
                    })
                }
                else return res;
        ...
        console.log(global.bot.logo.error + err)
    }
}
...
function buildAPI(globalOptions, html, jar) {
    ...
    if (html.indexOf("/checkpoint/block/?next") > -1) console.log(global.bot.logo.login + "Checkpoint terdeteksi, silakan masuk ke akun untuk memverifikasi.");

    const userID = maybeCookie[0].cookieString().split("=")[1].toString();
    console.log(global.bot.logo.login + `ID akun ${userID}.`);
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback ) {
  ...
  mainPromise
    .then(function () {
      console.log(global.bot.logo.login + ' berhasil masuk ke akun.');
      return callback(null, api);
    })
    .catch(function (e) {
      console.log(global.bot.logo.error + e.error || e);
      callback(e);
    });
}
